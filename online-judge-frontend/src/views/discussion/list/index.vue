...
type DiscussionItem = Api.Discuss.SelectDiscussListItem;
type BackendDiscussionItem = Omit<DiscussionItem, "User"> & {
    User?: Api.User.SimpleUserInfo | Api.User.SimpleUserInfo[] | null;
};
type UiDiscussionItem = Omit<DiscussionItem, "User"> & { User?: Api.User.SimpleUserInfo };

const route = useRoute();
const router = useRouter();

const list = ref<UiDiscussionItem[]>([]);
...
const buildSearchInfo = (): Api.Discuss.SelectDiscussListParams["SearchInfo"] | undefined => {
    if (!parentId.value && !userId.value) return undefined;
    return {
        ...(parentId.value ? { ParentId: parentId.value as Api.Discuss.DiscussId } : {}),
...
const normalizeIncoming = (incoming: BackendDiscussionItem[]): UiDiscussionItem[] => {
    return incoming.map((item) => {
        const rawUser = item.User;
        const user = Array.isArray(rawUser) ? rawUser[0] : (rawUser ?? undefined);
        return {
            ...(item as Omit<BackendDiscussionItem, "User">),
            User: user,
        };
    });
};

const appendUnique = (incoming: UiDiscussionItem[]) => {
...
const fetchPage = async (page: number): Promise<UiDiscussionItem[]> => {
    ...
    if (res.data.success && res.data.data) {
        ...
        return normalizeIncoming((res.data.data.List ?? []) as BackendDiscussionItem[]);
    }
...
const handleCardClick = (item: UiDiscussionItem) => {
...
<template>
    <section class="discussion-list-page" aria-label="讨论列表">
        <div class="header oj-glass-panel">
            <div class="header-left">
                <el-button class="title" type="primary" @click="handleWriteDiscuss">写讨论</el-button>
                <div class="subtitle" v-if="parentId || userId">
                    <span v-if="parentId" class="mono">Parent: {{ parentId }}</span>
        ...
        <div class="list-panel oj-glass-panel">
            <el-skeleton :loading="initialLoading" animated>
                ...
                <template #default>
                    <el-empty v-if="!list.length && !loadError" description="暂无讨论" />
                    ...
                    <div v-else class="discussion-list">
                        <el-card v-for="item in list" :key="String(item._id)" class="discussion-card" shadow="never">
                            <div class="discussion-card-inner" role="button" tabindex="0" @click="handleCardClick(item)"
                                ...
                                <div class="card-row">
                                    ...
                                    <div class="card-right">
                                        <div class="stat">
                                            ...
                                            <span class="stat-v">{{ item.Views ?? 0 }}</span>
                                        </div>
                                        <div class="stat">
                                            <el-icon class="stat-ic">
                                                <ChatRound />
                                            </el-icon>
                                            <span class="stat-v">{{ item.Comments ?? 0 }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
.discussion-card-inner:hover .card-title {
    color: var(--oj-color-primary);
}
